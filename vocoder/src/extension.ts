...
function deleteFromEditor(lines: boolean, start: number, end: number){
    const editor = vscode.window.activeTextEditor;
    if(!editor){
...
function grantMacExecutablePermission(folder: string, file: string){
    let cPath = shell.concat(folder);
    exec(`chmod +x ${file}`, {cwd: path.resolve(cwd, cPath)}, (error: any, stdout: any, stderr: any) => {
        if (error) {
